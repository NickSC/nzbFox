...
self.port.on('show', function () {
	resize();
	panelVisible = true;
	Tabs.forEach((Tab) => Tab.statusTimerInterval(self.options.prefs.refresh_active));
});
self.port.on('hide', function () {
	...
	Tabs.forEach((Tab) => Tab.statusTimerInterval(Tab.download.speed > 0 ? self.options.prefs.refresh_active : self.options.prefs.refresh_idle));
});
...
function Tab(type) { //// Tab Variables
	...
	// Buttons
	this.btnTogglePauseEle = this.content.find('#togglePause').button().click(() => _this.btnTogglePause_Click.call(_this));
	this.btnRefreshEle = this.content.find('#refresh').button({ icons: { primary: 'ui-icon-refresh' } }).click(() => _this.btnRefresh_Click.call(_this));
	this.btnShowMenuEle = this.content.find('#showMenu').button({ text: false, icons: { primary: 'ui-icon-triangle-1-n' } }).click(() => _this.btnShowMenu_Click.call(_this));
	this.btnOpenEle = this.content.find('#open').button({ text: false, icons: { primary: 'ui-icon-newwin' } }).click(() => _this.btnOpen_Click.call(_this)).click(() => self.port.emit('hide'));
	// Action Menu
	this.menuEle = this.content.find('ul#menu').menu({ position: { my: 'left bottom', at: 'right bottom' } }).hide();
	// Action Menu Items
	this.menuSpeedLimit = this.content.find('li#speedLimit').click(() => _this.menuSpeedLimit_Click.call(_this));

	this.menuPauseFor = this.content.find('li#pause-custom').click(() => _this.menuPauseFor_Click.call(_this));
	this.menuPauseFor5m = this.content.find('li#pause-5m').click(() => _this.pause.call(_this, 5));
	this.menuPauseFor15m = this.content.find('li#pause-15m').click(() => _this.pause.call(_this, 15));
	this.menuPauseFor30m = this.content.find('li#pause-30m').click(() => _this.pause.call(_this, 30));
	this.menuPauseFor60m = this.content.find('li#pause-60m').click(() => _this.pause.call(_this, 60));
...
Tab.prototype.btnRefresh_Click = () => this.refreshStatus();
...
Tab.prototype.btnShowMenu_Click = function () {
	...
	var _this = this
	$(document).one('click', () => _this.menuEle.hide());
	return false;
}
...
Tab.prototype.startStatusTimer = function () {
	...
	if (this.statusTimer_interval > 0)
		this.statusTimer = window.setTimeout(() => _this.refreshStatus.call(_this), this.statusTimer_interval * 1000);
}
Tab.prototype.startHistoryTimer = function () {
	this.stopHistoryTimer();
	var _this = this;
	this.historyTimer = window.setTimeout(() => _this.refreshHistory.call(_this), this.historyTimer_interval * 1000);
}
...
Tab.prototype.error = (message) => this.errorEle.html(message)
...
function sabTab(type) {
	...
	this.menuOnFinishNothing = this.content.find('li#finish-nothing').click(() => _this.setFinishAction.call(_this, ''));
	this.menuOnFinishShutdown = this.content.find('li#finish-shutdown').click(() => _this.setFinishAction.call(_this, 'shutdown_program'));
	this.menuOnFinishShutdownPC = this.content.find('li#finish-shutdownPC').click(() => _this.setFinishAction.call(_this, 'shutdown_pc'));
	this.menuOnFinishStandbyPC = this.content.find('li#finish-standbyPC').click(() => _this.setFinishAction.call(_this, 'standby_pc'));
	this.menuOnFinishHibernatePC = this.content.find('li#finish-hibernatePC').click(() => _this.setFinishAction.call(_this, 'hibernate_pc'));
	this.menuOnFinishScript = this.content.find('li#finish-script').click(() => _this.menuOnFinishScript_Click.call(_this));
...
$(function () {
	...
	$('button#showOptions').button({ text: false, icons: { primary: 'ui-icon-wrench' } }).click(() => self.port.emit('showOptions'));
