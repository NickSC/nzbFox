...
mPanel.port.on('hide', () => mPanel.hide());
mPanel.port.on('resize', ({ width, height }) => mPanel.resize(width, height));
mPanel.port.on('api-call', (data) => apiCall(mPanel, data));
...
function showNotification([type, title, icon, name, stats]) {
	...
	if (sp.prefs.dl_notification_type == 'standard' || sp.prefs.dl_notification_type == 'both') {
		let nPanel = Panel({
			...
			width: 1,
			height: 1, // automatically set
			onHide: () => nPanel.destroy()
		});
		nPanel.port.on('hide', () => nPanel.hide());
		nPanel.port.on('log', log);
		nPanel.port.on('resize', ({ width, height }) => nPanel.resize(width, height));
...
function createPageMod() {
	...
	PM = pageMod.PageMod({
		...
		onAttach: function (worker) {
			...
			worker.port.on('api-call', (data) => apiCall(worker, data));
